...
deg.dist.males <- c(0.365,
                    0.435,
                    0.17,
                    0.030
                    
)
deg.dist.females <- c(0.185,
                      0.793,
                      0.02,
                      0.002
...
tg.f.2 <- round(num.females*deg.dist.females[3],0)
tg.f.3 <- round(num.females*deg.dist.females[4],0)
#tg.f.4 <- round(num.females*deg.dist.males[5],0)
tg.m.2 <- round(num.females*deg.dist.males[3],0)
tg.m.3 <- round(num.females*deg.dist.males[4],0)
#tg.m.4 <- round(num.females*deg.dist.females[5],0)



target.stats <- c(tg.edges,tg.f.1,tg.f.2,tg.m.1,tg.m.2)


check_bip_degdist(num.females, num.males, deg.dist.females, deg.dist.males)


#ergm.fit <- ergm(nw ~ edges + b1degree(2:3) +b2degree(2:3),
#                 target.stats=target.stats)

ergm.fit <- ergm(nw ~ edges + b1degree(1:2) + b2degree(1:2),
                 target.stats=target.stats, constraints=~bd(maxout=3))
...
ergm.fit.gof <- gof(ergm.fit ~ degree+espartners+distance)

ergm.fit.gof
par(mfrow=c(2,2))
plot(ergm.fit.gof)

colMeans(attributes(netsim)$stats)

#Quantify or visualize the stochastic variation that occurs in the momentary degree distribution that occur in the realizations of this model.
par(mfrow=c(1,1))
...
sub1<-rbind(sim=colMeans(sim.stats), obs=target.stats)
...
matplot(1:nrow(sim.stats), sim.stats,
        pch=c("e","f1","f2","m1","m2"), cex=.65,
        main="100 simulations from ergm.fit model", sub="(MCMC.interval=10000)",
        xlab="Replicate", ylab="frequency")
abline(h=target.stats, col=c(1:5))
abline(h=1.1*target.stats,col=c(1:5),lty=2)
abline(h=0.9*target.stats,col=c(1:5),lty=2)
...
subs = rbind(sim=colMeans(sim.deg), obs=c(tg.f.0+tg.m.0,tg.f.1+tg.m.1,tg.f.2+tg.m.2,tg.f.3+tg.m.3))


matplot(1:nrow(sim.deg), sim.deg, pch=as.character(0:3), cex=.5,
        main="Comparing ergm.sims to non-targeted degree frequencies",
        sub = "(Total edges and degree 1 and 2 targeted)",
        xlab = "Replicate", ylab = "Frequencies")
abline(h=c(tg.f.0+tg.m.0,tg.f.1+tg.m.1,tg.f.2+tg.m.2,tg.f.3+tg.m.3), col=c(1:4))

sim.deg2 <- sim.deg[,1] - (tg.f.0+tg.m.0)
sessionInfo()
getOption("pkgType") 
...
#Diagnostic:
est1 <- netest(nw,formation = ~edges + b1degree(1:2) + b2degree(1:2),target.stats,
