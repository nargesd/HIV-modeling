...
ergm.fit <- ergm(nw ~ edges + b1degree(1:2) + b2degree(1:2), verbose=TRUE,
                 target.stats=target.stats,
                 control=control.ergm(MCMC.samplesize=1e+5,MCMC.interval=1000,
                                      MCMLE.maxit = 5,
                                      MCMC.burnin = 1e+5,
                                      MCMLE.steplength = 0.25,seed=125))
...
rbind(sim=colMeans(sim.deg), obs=c(tg.f.0+tg.m.0,tg.f.1+tg.m.1,tg.f.2+tg.m.2,tg.f.3+tg.m.3))
...
param <- param.net(trans.rate=0.01, act.rate=2)
