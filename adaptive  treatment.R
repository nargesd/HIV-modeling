...
while (t<= tmax){
  ...
  else if ((t>=treat.init) & (main[t-1,2]<= CD4thresh.l) & (main[t-1,2] <= main[t-2,2]))
  ...
  else if (t>=treat.init & main[t-1,2]>= CD4thresh.u & main[t-2,2] <= main[t-1,2])
  {milestone=rbind(milestone,t)
   Y0=main[t-1,2:5]
   timevec1=seq(t-1,t)
   odeoutput=lsoda(Y0,timevec1,no.treatment,parms="",atol=1e-5)
